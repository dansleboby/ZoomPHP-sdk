...
/**
 * DashboardWebinarParticipants200ResponseAllOfParticipantsInner Class Doc Comment
 *
 * @category Class
 * @package  Zoom\Api
 * @author   OpenAPI Generator team
 ...
 */
class DashboardWebinarParticipants200ResponseAllOfParticipantsInner implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'dashboardWebinarParticipants_200_response_allOf_participants_inner';
    ...
    public const LEAVE_REASON__NAME_LEFT_THE_WEBINAR = '$name left the webinar.';
    public const LEAVE_REASON__NAME_GOT_DISCONNECTED_FROM_THE_WEBINAR = '$name got disconnected from the webinar.';
    public const LEAVE_REASON_HOST_ENDED_THE_WEBINAR = 'Host ended the webinar.';
    public const LEAVE_REASON_HOST_CLOSED_THE_WEBINAR = 'Host closed the webinar.';
    public const LEAVE_REASON_HOST_STARTED_A_NEW_WEBINAR = 'Host started a new webinar.';
    ...
    public const LEAVE_REASON_EXCEEDED_FREE_WEBINAR_MINUTES_LIMIT = 'Exceeded free webinar minutes limit.';
    ...
    {
        return [
            self::LEAVE_REASON__NAME_LEFT_THE_WEBINAR,
            self::LEAVE_REASON__NAME_GOT_DISCONNECTED_FROM_THE_WEBINAR,
            self::LEAVE_REASON_HOST_ENDED_THE_WEBINAR,
            self::LEAVE_REASON_HOST_CLOSED_THE_WEBINAR,
            self::LEAVE_REASON_HOST_STARTED_A_NEW_WEBINAR,
            self::LEAVE_REASON_NETWORK_CONNECTION_ERROR,
            self::LEAVE_REASON_HOST_DID_NOT_JOIN,
            self::LEAVE_REASON_EXCEEDED_FREE_WEBINAR_MINUTES_LIMIT,
    ...
    public function setCustomerKey($customer_key)
    {
        if (!is_null($customer_key) && (mb_strlen($customer_key) > 15)) {
            throw new \InvalidArgumentException('invalid length for $customer_key when calling DashboardWebinarParticipants200ResponseAllOfParticipantsInner., must be smaller than or equal to 15.');
        }
     ...
     * Sets data_center
     *
     * @param string|null $data_center The data center where participant's webinar data is stored.
     *
     * @return self
     */
     ...
     * Sets join_time
     *
     * @param \DateTime|null $join_time The time at which participant joined the webinar.
     *
     * @return self
     */
     ...
     * Sets leave_reason
     *
     * @param string|null $leave_reason The reason why the participant left the webinar, where `$name` is the participant's username:  * `$name left the meeting.`  * `$name got disconnected from the meeting.`  * `Host ended the meeting.`  * `Host closed the meeting.`  * `Host started a new meeting.`  * `Network connection error.`  * `Host did not join.`  * `Exceeded free meeting minutes limit.`  * `Removed by host.`  * `Unknown reason.`  * `Leave waiting room.`  * `Removed by host from waiting room.`
     *
     * @return self
     */
     ...
     * Sets leave_time
     *
     * @param \DateTime|null $leave_time The time at which a participant left the webinar. For live webinars, this field will only return if a participant has left the ongoing webinar.
     *
     * @return self
     */
     ...
     * Sets microphone
     *
     * @param string|null $microphone The type of microphone that the participant used during the webinar.   **Note:** This response returns an empty string (`““`) value for any users who are **not** a part of the host's account (external users).
     *
     * @return self
     */
     ...
     * Sets recording
     *
     * @param bool|null $recording Whether the recording feature was used during the webinar.
     *
     * @return self
     */
     ...
     * Sets role
     *
     * @param string|null $role The participant's role:  * `host` — Host.  * `attendee` — Attendee.  * `panelist` — Panelist.
     *
     * @return self
     */
     ...
     * Sets speaker
     *
     * @param string|null $speaker The type of speaker that the participant used during the webinar.   **Note:** This response returns an empty string (`““`) value for any users who are **not** a part of the host's account (external users).
     *
     * @return self
     */
     ...
     * Sets user_id
     *
     * @param string|null $user_id The participant's ID. This value assigned to a participant upon joining a webinar and is only valid for the webinar's duration.
     *
     * @return self
     */
